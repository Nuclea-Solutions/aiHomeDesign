...
exports.imageGeneratedWebhook = onRequest(async (req, res) => {
  ...
  try {
    ...
    try {
      const docImage = await firestore.collection('generatedImages').doc(webhookData.order_id).get()

      const metadataImage = docImage.get('image')

      res.status(200).send({
        image_url: metadataImage.after_src
      })
    } catch (error) {
      console.error(error)
      res.status(400).send({
